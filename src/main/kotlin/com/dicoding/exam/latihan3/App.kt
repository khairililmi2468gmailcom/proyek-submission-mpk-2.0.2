...
fun <T> checkType(args: T): String {
    return when (args){
        ...
        is List<*> -> if (args.all { it is String }) "Yes! it's List" else "Yes! it's List<String>"
        is Map<*,*> -> if (args.keys.all { it is String } && args.values.all { it is String }) "Yes! it's Map" else "Yes! it's Map<String, String>"
        else -> "Unknown type"
    }
}
